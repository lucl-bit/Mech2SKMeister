...
const InertiaMath = (function () {
  ...
  function partIyOwn(p) {
    ...
    if (p.type === 'triangle') {
      // orient 'up'/'down': Basis horizontal → I_y = b·h³/36
      // orient 'right'/'left': Basis vertikal, h = z-Ausdehnung → I_y = b·h³/48
      if (p.orient === 'right' || p.orient === 'left') {
        return (p.b * Math.pow(p.h, 3)) / 48;
      }
      return (p.b * Math.pow(p.h, 3)) / 36;
    }
...
const InertiaFixtures = [
  ...
  // T-Profil
  // Schwerpunkt-Herleitung: A_flansch=3, A_steg=2, z_c=(3·2.5 + 2·1.0)/5 = 1.9 von Steg-Unterkante
  // → cz_flansch = 2.5−1.9 = 0.6 a, cz_steg = 1.0−1.9 = −0.9 a
  {
    id: 'T-profile',
    title: 'T-Profil',
    level: 'mid',
    parts: [
      { type: 'rect', b: 3, h: 1, cy: 0, cz: +0.6, sign: +1 }, // Flansch
      { type: 'rect', b: 1, h: 2, cy: 0, cz: -0.9, sign: +1 }, // Steg
    ],
...
function tProfileOutline(bf, hf, bw, hw) {
  // Schwerpunkt berechnen (von Steg-Unterkante aus):
  //   z_c = (bf·hf·(hw + hf/2) + bw·hw·(hw/2)) / (bf·hf + bw·hw)
  const Af = bf * hf, Aw = bw * hw;
  const zc = (Af * (hw + hf / 2) + Aw * (hw / 2)) / (Af + Aw);
  const midBot = -zc;           // Steg-Unterkante relativ zum Schwerpunkt
  const midTop = hw - zc;       // Steg-Oberkante = Flansch-Unterkante
  const top    = hw + hf - zc;  // Flansch-Oberkante
  return [
